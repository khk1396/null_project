	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (selectProductId != null) {	// 교환을 위해 상품하나를 선택해서 상품등록페이지에 왔을때
			...
			if(productId.length > 1 || productId.length == 0) {	// 내 상품 체크박스를 2개이상 체크했을시 오류페이지로
				...
				trade.setOtherProductId(selectProductId);
				trade.setMyProductId(myProduct.getProductId());
				trade.setMyProductUserPk(myProduct.getUserPk());
				trade.setMyComment(request.getParameter(productId[0]));		// Comment를 name값을 상품식별자로 받아옴
				System.out.println(trade);
				int result = marketService.updateTradeApply(trade);
				
				if(result > 0) {	// 성공
					System.out.println("교환 신청 성공");
					request.setAttribute("alertMsg", "교환 신청 되었습니다.");
				}else {
					System.out.println("교환 신청 실패");
				}
				
				request.getRequestDispatcher("/tradeRegist").forward(request, response);
			}
